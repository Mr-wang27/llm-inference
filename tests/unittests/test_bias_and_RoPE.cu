...
int main() {
    ...
    int* padding_offset = (int*)malloc(sizeof(int) * batch_size * seq_len);     // [batch_size, seq_len]: 一个句子的padding offset
    int* history_length = (int*)malloc(sizeof(int) * batch_size);               // [batch_size]:每个句子的上下文历史长度
    int* input_length = (int*)malloc(sizeof(int) * batch_size);                 // [batch_size]:输入的每个句子的长度
    const int token_num = batch_size * seq_len;                                 // token_num = batch_size*seq_len
    ...
    float* q = (float*)malloc(sizeof(float) * batch_size * seq_len * head_num * head_size); //output        q:[batch_size, seq_len, head_num, head_size]
    float* k = (float*)malloc(sizeof(float) * batch_size * seq_len * kv_head_num * head_size); //output     k:[batch_size, seq_len, kv_head_num, head_size]
    float* v = (float*)malloc(sizeof(float) * batch_size * seq_len * kv_head_num * head_size); //output     v:[batch_size, seq_len, kv_head_num, head_size]
    float* QKV = (float*)malloc(sizeof(float) * token_num * (head_num + 2 * kv_head_num) * head_size);//    QKV:[token_num, qkv_head_num, head_size]
    float* qkv_bias = (float*)malloc(sizeof(float) * (head_num + 2 * kv_head_num) * head_size);       //    qkv_bias:[qkv_head_num, head_size]
